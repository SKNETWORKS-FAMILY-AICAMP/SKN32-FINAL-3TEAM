...
def build() -> tuple[list[dict], dict]:
    ...
    stat: dict = {"사례집": 0, "의결서": 0, "충돌": collections.Counter()}

    for r in _jsonl(CASEBOOK):
        types = r.get("확정유형") or []
        ho = r.get("호")
        hos = ho if isinstance(ho, list) else ([ho] if ho else [])
        art = " · ".join(CASEBOOK_ARTICLE.format(호=h) for h in hos) or "식품표시광고법 제8조제1항"
        for q in r.get("인용표현") or []:
            n = norm(q)
            if len(n) < MIN_TERM or not types:
                continue
            e = entries.setdefault(
                n, {"term": n, "원문": [], "유형": set(), "근거": set(), "출처": set()}
            )
            e["원문"].append(str(q))
            e["유형"] |= set(types)
            e["근거"].add(art)
            e["출처"].add("mfds_casebook")
            stat["사례집"] += 1
    ...
    for r in json.loads(FTC.read_text(encoding="utf-8")):
        ...
        for q in r.get("문구") or []:
            ...
            for u in units:
                e["유형"].add(u["label"])
                e["근거"].add(f"{FTC_LAW} {u['article']}")
            e["출처"].add("ftc_decisions_body")
            stat["의결서"] += 1
