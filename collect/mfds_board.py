...
🚨 **PDF 만 받는다.** 같은 자료가 `.hwpx` 와 `.pdf` 로 두 벌 붙는 일이 흔하고 내용이 같다.
...
def collect(source_id: str, *, use: str, dry_run: bool) -> tuple[int, int, int]:
    ...
    for idx, (name, href) in enumerate(links, 1):
        ...
        try:
            ...
            failed += 1
            continue
        if len(body) < MIN_PDF or not body.startswith(b"%PDF"):
            head = body[:16].decode("latin-1", "replace")
            print(f"  ❌ {name[:46]} — PDF 가 아니다 ({len(body):,} B · {head!r})")
            failed += 1
            continue
