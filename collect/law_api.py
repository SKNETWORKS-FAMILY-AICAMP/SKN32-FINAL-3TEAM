...
#     🚨 `prec` 전량은 **172,343건**(2026-09-18 실측 · 사용자 실행) — 전량 수집은 불가라 좁힌다.
#        `decc` 는 아직 전량 규모를 재지 않았다 — 다음 사람이 할 일이다.
#
...
#     🚨 `decc` 에도 같은 질의가 간다 (`QUERIES` 는 두 target 공용). **`decc` 순증은 재지 않았다.**
#        재수집 때 `--dry-run` 으로 decc 순증을 먼저 보고, 무관이 많으면 target 별 질의로 나눈다.
QUERIES: tuple[str, ...] = (
    "표시광고",
    "부당한 광고",
    "건강기능식품",
