...
--    | ck_sanction_four_eyes             | 서명 0명 / 1명                      | **통과** |
...
--
-- 🚨 지금 `chunk` 와 `sanction_rule` 은 0행이라 SET NOT NULL 이 안전하다. 적재가 시작된
--    뒤에는 이 마이그레이션이 못 돈다 — **적재 전에 거는 것이 이 파일의 요점이다.**
...
-- ── ② sanction_rule 2인 확인 (D-66) ───────────────────────────────────
--    ⛔ 종전 `verified_by IS NULL OR reviewed_by IS NULL OR verified_by <> reviewed_by` 는
--       **아무도 서명하지 않은 행**과 **한 사람만 서명한 행**을 통과시켰다.
--       같은 사람이 두 칸에 들어간 경우만 막혔다 — 실제로 일어나지 않는 경우다.
--    🚨 병합 셀 상속 행이 별표7 71행 중 48행이다. 2인 대조가 필요한 바로 그 자리를
--       제약이 안 지키고 있었다.
DO $$
DECLARE n bigint;
BEGIN
  SELECT count(*) INTO n FROM sanction_rule WHERE verified_by IS NULL OR reviewed_by IS NULL;
  IF n > 0 THEN
    RAISE EXCEPTION '🔴 sanction_rule 에 서명이 빈 행이 %건 있다 — 2인 확인을 먼저 마친다 (D-66)', n;
  END IF;
END $$;
...
COMMENT ON CONSTRAINT ck_sanction_four_eyes ON sanction_rule IS
  '2인 확인 (D-66). ck_source_four_eyes 와 같은 모양 — 컬럼이 NOT NULL 이라 널 우회가 없다 (0006).';
