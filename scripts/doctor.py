...
def check_data(*, verify_hash: bool) -> int:
    ...
    if dups:
        ...
        if split:
            ...
            print("     규약 2(덮어쓰지 않는다)가 클론 안에서만 성립한다는 뜻이다.")
            print("     🚨 원본을 가진 클론은 다음 수집에서 StoreError 로 멈춘다.")
            ...
            print("     고치는 법 — 어느 쪽이 기준인지 **사람이 정하고** 원장에 근거를 적는다.")
            print("     원천이 조용히 고친 것인지 응답에 유동 필드가 있는지부터 확인한다.")
