...
CANDIDATE_TYPES = [
    # 🔄 2026-09-20 (D-255 · 팀장) — **편입 대기가 아니라 범위 밖이다.** 뒷광고의 위반은 문구가 아니라
    #    「경제적 이해관계 표시가 **없는 것**」(추천·보증 심사지침 Ⅴ.6)이라 광고 문구만 받는 판정기가 볼 수 없다.
    #    ⛔ 30건을 넘겨도 편입하지 않는다 — 건수가 아니라 **입력에 없는 사실**이 이유다.
    #    ★ 체험기 **형식** 자체가 위반인 식품 광고(시행령 [별표 1] 5.다)는 `후기_체험기_기만` 이 맡는다.
    "추천_보증_뒷광고",
    "부당_비교광고",
    # 🔄 2026-09-08 추가 (D-151 · 팀장 확정) — 표시광고법 제3조 제1항 **제4호**다.
    #    ⛔ 네 호 중 이것만 목록에서 빠져 있었다. `preprocess/ftc_extract.py` 는
    #       이미 `비방광고` 를 뽑고 있었는데(6건) **그 유형이 정식 목록에 없었다** —
    #       뽑는 쪽과 인정하는 쪽이 어긋난 채로 지나갔다.
    #    ★ 해설서의 「부당한 비교ㆍ비방」 139건이 이 자리를 채운다.
    "비방광고",
    "실증책임_위반",
    # 🔄 2026-09-10 등재 (D-65 · 0005) — 화장품법 시행규칙 [별표 5] 제2호 **나목**.
    #    ⛔ `질병_예방치료_표방`·`건강기능식품_오인`·`의약품_오인` 셋은 식품표시광고법
    #       제8조제1항 1~3호 구조라 **식품 축**이다. 화장품 축의 같은 자리(기능성화장품
    #       오인)가 비어 있어, `violation_article` 에 별표5 나목을 적을 수가 없었다.
    #    🔴 등재이지 편입이 아니다 — 지금 평가셋은 10유형 중 8유형이 30건 미달이라
    #       D-65 의 「확장 금지」가 걸려 있다. `VIOLATION_TYPES` 승격 판정일은 09-17 이다.
    "기능성화장품_오인",
]
