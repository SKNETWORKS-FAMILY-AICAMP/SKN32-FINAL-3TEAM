"""scripts/experiment.py — T2·T3 실험의 **분배 단위에 형식을 준다** (2026-09-12 밤 · D-94).
...
★ **그래서 파일을 두 층으로 가른다.**
    ...
    experiments/harness.yaml     팀장이 고정한다 — split · 시드 · 평가 · 무변화 3회
...
⬜ **필드 목록은 아직 판정 대상이다** (병렬작업 계약 §8 ④) — 무엇을 팀원에게 **열고**
   ...
   `params` 의 허용 키는 팀장이 `harness.yaml` 에서 정한다.
"""
...
HARNESS_FILE = EXPERIMENTS / "harness.yaml"
...
class ExperimentConfig(BaseModel):
    ...
    track: str = Field(pattern=r"^T[23]$")
    ...
    def _no_locked_keys(self) -> ExperimentConfig:
        ...
        if bad:
            raise ValueError(
                ...
                "  바꿔야 할 이유가 있으면 `experiments/harness.yaml` 을 팀장이 고친다"
            )
        return self


def load_harness(path: Path = HARNESS_FILE) -> Harness:
    """🚨 없으면 **멈춘다** — 「없음」이 기본값으로 집계되지 않게 (D-72)."""
    if not path.exists():
        raise SystemExit(
            f"🔴 harness 가 없다 — {path}\n"
            "  D-94: split · 시드 · 평가 스크립트 · 기록 포맷은 **팀장이 고정한다.**\n"
            "  🚨 마감은 2W 말이다. 늦으면 팀원 4명이 그대로 대기한다.\n"
...
def load_config(path: Path, harness: Harness) -> ExperimentConfig:
    ...
    if unknown:
        raise ValueError(
            ...
            "  🚨 여는 것은 팀장 판정이다 — `experiments/harness.yaml` 의 `allowed_params`"
        )
    return cfg
