...
#: 🔄 2026-09-24 (D-283) — **이 8유형으로 붙인 라벨은 평가에 쓰지 않는다.** 조문 근거가 없는 자체 분류다 —
#:    6·7번이 법 제8조①6(비방)·7(부당 비교)호와 **반대**이고, 5호의 목 11개(체험기·의사 추천·외국어 남용 …)가 한 줄이다.
#:    ★ 라벨의 정본은 조문 인용이다(`collect/statute.py` · D-282). 이 표는 이미 붙인 라벨을 읽고 되돌리는 데만 남긴다.
TYPES: tuple[str, ...] = (
    "질병_예방치료_표방",  # 1
    "의약품_오인",  # 2
    "건강기능식품_오인",  # 3
