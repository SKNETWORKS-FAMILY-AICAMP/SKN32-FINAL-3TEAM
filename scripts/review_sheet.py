...
def main() -> None:
    ...
    L: list[str] = [
        ...
        "| 1-2 | 🔄 **`reviewed_at` 은 찍고 `dissent_note` 를 남깁니다** — 「보고 반대함」과"
        " 「아직 안 봄」이 둘 다 빈칸이면 진척과 이견이 같은 모양으로 보입니다."
        " 게이트는 그대로 `reviewed_by` 만 봅니다 |",
        ...
        "",
        "> 🚨 **정정 이력(+2)만으로는 A 가 되지 않습니다.** B 에 있는 소스에도 정정 이력이"
        " 있을 수 있으니, B 의 판정 근거를 건너뛰지 마십시오.",
        "",
    ]
    for k, s in a:
