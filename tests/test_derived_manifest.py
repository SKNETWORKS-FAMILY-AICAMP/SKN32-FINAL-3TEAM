...
# ══════════════════════════════════════════════════════════
# 🆕 D-254 — `--write` 가 원천·표본을 조용히 버리지 않는다 (런처 전수 감사 §1-3)
# ══════════════════════════════════════════════════════════
def _fake(tmp_path: pathlib.Path, mp: pytest.MonkeyPatch, files: dict[str, bytes]) -> pathlib.Path:
    root = tmp_path / "B"
...
def _main(mp: pytest.MonkeyPatch, *args: str) -> int:
    mp.setattr(sys, "argv", ["derived_manifest.py", *args])
    return dm.main()


LABEL = "labels/누군가.jsonl"  # 원천
SHEET = "golden/round2_labelsheet.jsonl"  # 표본
GEN = "golden/golden.jsonl"  # 생성물


@pytest.mark.gate
@pytest.mark.parametrize("rel", [LABEL, SHEET])
def test_쓰기는_원천_표본이_빠지거나_바뀌면_멈춘다(
    tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch, rel: str
) -> None:
    """🔴 라벨이 지워져도 `--write` 가 원장에서 **조용히** 뺐다 → publish 초록. `data-refresh` 가 자동으로 돈다."""
    root = _fake(tmp_path, monkeypatch, {LABEL: b'{"l":1}\n', SHEET: b'{"s":1}\n', GEN: b"{}\n"})
    assert _main(monkeypatch, "--write") == 0
    before = dm.OUT.read_bytes()
    (root / "data" / "derived" / rel).write_bytes(b'{"x":2}\n')  # 바뀜
    assert _main(monkeypatch, "--write") == 1
    (root / "data" / "derived" / rel).unlink()  # 없어짐
    assert _main(monkeypatch, "--write") == 1
    assert dm.OUT.read_bytes() == before, "멈췄는데 원장을 썼다"
    # 사람이 경로를 적어 받아들이면 쓴다
    assert _main(monkeypatch, "--write", "--accept-loss", f"data/derived/{rel}") == 0
    assert f"data/derived/{rel}" not in dm.ledger()


@pytest.mark.gate
def test_쓰기는_생성물_변화와_새_표본은_막지_않는다(
    tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """★ 라벨 판(round 시트)은 정당하게 **새로 생긴다** — 추가는 자유 · 생성물은 명령이 다시 만든다."""
    root = _fake(tmp_path, monkeypatch, {LABEL: b'{"l":1}\n', GEN: b"{}\n"})
    assert _main(monkeypatch, "--write") == 0
    (root / "data" / "derived" / GEN).write_bytes(b'{"v":2}\n')
    (root / "data" / "derived" / SHEET).write_bytes(b'{"s":1}\n')
    assert _main(monkeypatch, "--write") == 0
    assert SHEET in {k.removeprefix("data/derived/") for k in dm.ledger()}
    (root / "data" / "derived" / GEN).unlink()
    assert _main(monkeypatch, "--write") == 0


@pytest.mark.gate
def test_대조가_원천이_없으면_write_가_아니라_되살리기를_권한다(
    tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = _fake(tmp_path, monkeypatch, {LABEL: b'{"l":1}\n', GEN: b"{}\n"})
    assert _main(monkeypatch, "--write") == 0
    (root / "data" / "derived" / LABEL).unlink()
    capsys.readouterr()
    assert _main(monkeypatch, "--check") == 1
    err = capsys.readouterr().err
    assert "되살린다" in err and "갱신하려면 --write" not in err, err


@pytest.mark.gate
def test_모르는_부류는_잃으면_멈춘다() -> None:
    """🚨 옛 원장의 부류가 표에 없는 이름이면 막는 쪽이다 (D-220)."""
    old = {
        "data/derived/x.jsonl": {"경로": "data/derived/x.jsonl", "부류": "새부류", "sha256": "a"}
    }
