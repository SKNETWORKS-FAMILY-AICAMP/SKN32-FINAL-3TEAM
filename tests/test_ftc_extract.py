...
def test_phrase_keeps_numbers_and_urls() -> None:
    """🔴 **저장되는 문구는 원문이어야 한다** (D-152).

    ⛔ `sep_norm` 을 지난 문장으로 문구를 뽑고 있었다. 실측 — 627문구 중 **70건이 망가졌다**:
    ...
    ★ `preprocess/text.py` 가 「매칭 직전에만 쓴다 · 원문을 보관한다」고 적어 뒀는데
      호출부가 어겼다 — 모듈이 규칙을 적어 두는 것만으로는 안 막힌다.
    """
    q = "적중, 합격의 절대기준! 2015 명중률 97.5%"
    assert phrases_in(f"‘{q}’ 라고 광고하였다") == [q]
    q2 = "유튜브 사이트(www.youtube.com)에서 확인"
    assert phrases_in(f"‘{q2}’ 라고 표시") == [q2]
